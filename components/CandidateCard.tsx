...
const CandidateCard: React.FC<CandidateCardProps> = ({ candidate, onViewProfile }) => {
    const { userProfile, skills, jobPreferences } = candidate;
    return (
        <div className="bg-surface dark:bg-dark-surface p-5 rounded-xl border border-border dark:border-dark-border animate-fade-in-up transition-all hover:shadow-lg hover:border-primary/50 hover:scale-[1.02]">
            <div className="flex items-start gap-4">
                <img src={userProfile.profilePhoto || ''} alt={userProfile.name} className="w-16 h-16 rounded-full object-cover ring-2 ring-slate-200 dark:ring-slate-700" />
                <div className="flex-1">
                    <h3 className="font-bold text-lg text-text-primary dark:text-dark-text-primary">{userProfile.name}</h3>
                    <p className="text-primary dark:text-primary-light font-medium text-sm">{userProfile.title}</p>
                    <div className="flex items-center gap-2 mt-1 text-xs text-text-secondary dark:text-dark-text-secondary">
                        <MapPinIcon className="w-3.5 h-3.5" />
                        <span>{jobPreferences.location}</span>
                    </div>
                </div>
