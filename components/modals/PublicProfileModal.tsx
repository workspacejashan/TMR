...
const PublicProfileModal: React.FC<PublicProfileModalProps> = ({ isOpen, onClose, candidateProfile, openModal, openConnectModal, isRecruiterView = false }) => {
    ...
    const { userProfile, jobPreferences, skills, availability, documents } = candidateProfile;

    const publicDocs = documents.filter(doc => doc.visibility === 'public');
    const gatedDocs = documents.filter(doc => doc.visibility === 'gated');
    const profileLink = `https://thatsmyrecruiter.com/p/${userProfile.name.toLowerCase().replace(/\s/g, '-') || 'your-name'}`;
                    ...
                    <aside className="w-full md:w-1/3 lg:w-1/4">
                        <div className="space-y-6 text-center md:text-left">
                            <div className="flex flex-col items-center">
                                ...
                                <h1 className="text-2xl font-bold text-text-primary dark:text-dark-text-primary mt-4">{userProfile.name || 'Your Name'}</h1>
                                <p className="text-md text-primary dark:text-primary-light font-medium">{userProfile.title || 'Your Professional Title'}</p>
                            </div>
                            <div className="space-y-3">
                                <DetailItem icon={<MapPinIcon className="w-4 h-4"/>}>{jobPreferences.location || <span className="italic text-slate-400">Not specified</span>}</DetailItem>
                                <DetailItem icon={<div className="w-4 flex justify-center">{availability.contactMethods.map(m => m === 'call' ? <PhoneIcon key="c" className="w-4 h-4 mr-1"/> : <ChatBubbleLeftEllipsisIcon key="t" className="w-4 h-4"/>)}</div>}>
                                    {availability.contactMethods.map(m => m.charAt(0).toUpperCase() + m.slice(1)).join(' & ') || <span className="italic text-slate-400">Not specified</span>}
                                </DetailItem>
                            </div>
                            {!isRecruiterView && (
                            <div>
                    ...
                    <main className="w-full md:w-2/3 lg:w-3/4 space-y-8">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="bg-slate-50 dark:bg-dark-surface p-4 rounded-xl border border-border dark:border-dark-border">
                                    <DetailItem icon={<i className="fa-solid fa-user-doctor w-4 text-center"></i>} label="Preferred Role(s)">
                                        ...
                                        {jobPreferences.roles.length > 0 ? jobPreferences.roles.map(role => (
                                ...
                                <div className="bg-slate-50 dark:bg-dark-surface p-4 rounded-xl border border-border dark:border-dark-border space-y-3">
                                    <DetailItem icon={<CalendarDaysIcon className="w-4 h-4"/>} label="Shift">{jobPreferences.shift || <span className="italic text-slate-400">Not specified</span>}</DetailItem>
                                    <DetailItem icon={<CurrencyDollarIcon className="w-4 h-4"/>} label="Pay">{jobPreferences.payExpectations || <span className="italic text-slate-400">Not specified</span>}</DetailItem>
                                </div>
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="bg-slate-50 dark:bg-dark-surface p-4 rounded-xl border border-border dark:border-dark-border grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <DetailItem icon={<GlobeAltIcon className="w-4 h-4"/>} label="Time Zone">{availability.timeZone || <span className="italic text-slate-400">Not specified</span>}</DetailItem>
                                    <DetailItem icon={<ClockIcon className="w-4 h-4"/>} label="Working Hours">{availability.workingHours || <span className="italic text-slate-400">Not specified</span>}</DetailItem>
                            </div>
                        </section>
