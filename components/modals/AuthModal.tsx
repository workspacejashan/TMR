...
import { BotIcon, GoogleIcon, ArrowLeftIcon, BuildingOfficeIcon, BriefcaseIcon } from '../icons/Icons';
...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onLogin, onSignUp }) => {
                ...
                <div className="space-y-6 animate-fade-in-up">
                    <form onSubmit={handleLoginSubmit} className="space-y-4">
                        <div>
                            ...
                            <input type="email" id="login-email" value={email} onChange={e => setEmail(e.target.value)} required className="block w-full bg-background dark:bg-dark-surface border-border dark:border-dark-border rounded-lg shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-offset-surface dark:focus:ring-offset-dark-surface focus:ring-primary focus:border-primary sm:text-sm p-3 transition" placeholder="you@example.com or recruiter@acme.com" />
                        </div>
                        <div>
                        ...
                        <button type="submit" className="w-full bg-primary-gradient text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:ring-4 focus:ring-primary/50 transition-all duration-300 transform hover:scale-105 shadow-md">Login</button>
                    </form>
                    <div className="relative">
                        <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border dark:border-dark-border" /></div>
                        <div className="relative flex justify-center text-sm"><span className="bg-surface dark:bg-dark-surface px-2 text-text-secondary dark:text-dark-text-secondary">OR</span></div>
                    </div>
                    <button onClick={() => onLogin(email)} className="w-full flex items-center justify-center gap-3 bg-white dark:bg-dark-surface hover:bg-slate-50 dark:hover:bg-slate-800 border border-border dark:border-dark-border text-slate-700 dark:text-slate-200 font-semibold py-3 px-4 rounded-lg focus:outline-none focus:ring-4 focus:ring-slate-500/30 transition-all duration-300">
            ...
            {view === 'signup' && (
                 <div className="space-y-6 animate-fade-in-up">
                    <form onSubmit={handleSignUpSubmit} className="space-y-4">
                        ...
                        <button type="submit" className="w-full bg-primary-gradient text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:ring-4 focus:ring-primary/50 transition-all duration-300 transform hover:scale-105 shadow-md">Create Account</button>
                    </form>
                    <div className="relative">
                        <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border dark:border-dark-border" /></div>
                        <div className="relative flex justify-center text-sm"><span className="bg-surface dark:bg-dark-surface px-2 text-text-secondary dark:text-dark-text-secondary">OR</span></div>
                    </div>
                    <button onClick={() => onSignUp(selectedUserType)} className="w-full flex items-center justify-center gap-3 bg-white dark:bg-dark-surface hover:bg-slate-50 dark:hover:bg-slate-800 border border-border dark:border-dark-border text-slate-700 dark:text-slate-200 font-semibold py-3 px-4 rounded-lg focus:outline-none focus:ring-4 focus:ring-slate-500/30 transition-all duration-300">
                        <GoogleIcon /> Continue with Google
                    </button>
                    <p className="text-center text-sm text-text-secondary dark:text-dark-text-secondary">
