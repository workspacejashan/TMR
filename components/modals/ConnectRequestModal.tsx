...
const ConnectRequestModal: React.FC<ConnectRequestModalProps> = ({ isOpen, onClose, candidate, onSend }) => {
  const [message, setMessage] = useState(`Hi ${candidate.userProfile.name}, I came across your profile and was impressed with your experience. I'm hiring for a role that seems like a great fit and would love to connect to discuss it further.`);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Connect with ${candidate.userProfile.name}`}>
      <div className="space-y-6">
        <div className="flex items-center gap-4 p-3 bg-slate-50 dark:bg-dark-surface/50 rounded-lg border border-border dark:border-dark-border">
          <img src={candidate.userProfile.profilePhoto || ''} alt={candidate.userProfile.name} className="w-14 h-14 rounded-full object-cover" />
          <div>
            <h3 className="font-bold text-text-primary dark:text-dark-text-primary">{candidate.userProfile.name}</h3>
            <p className="text-sm text-primary dark:text-primary-light">{candidate.userProfile.title}</p>
          </div>
        </div>
        <div>
