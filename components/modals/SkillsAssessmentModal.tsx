...
const SkillsAssessmentModal: React.FC<SkillsAssessmentModalProps> = ({ isOpen, onClose, skills, setSkills }) => {
  ...
  const handleAddSkill = () => {
      if (currentSkillName.trim() && !skills.some(s => s.name.toLowerCase() === currentSkillName.trim().toLowerCase())) {
          setSkills([...skills, { name: currentSkillName.trim(), level: currentSkillLevel }]);
          setCurrentSkillName('');
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Manage Your Skills">
      <div className="space-y-6">
        <div className="space-y-3 max-h-48 overflow-y-auto pr-2 -mr-3">
            ...
            {skills.length === 0 && <p className="text-sm text-center text-text-secondary dark:text-dark-text-secondary py-4">No skills added yet. Use the form below to add one.</p>}
        </div>
        ...
        <div className="pt-2">
          <button
            onClick={onClose}
            className="w-full bg-primary-gradient text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:ring-4 focus:ring-primary/50 transition-all duration-300 transform hover:scale-105 shadow-md hover:shadow-lg"
          >
            Done
          </button>
        </div>
