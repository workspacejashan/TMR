...
const DocumentUploadModal: React.FC<DocumentUploadModalProps> = ({ isOpen, onClose, documents, setDocuments }) => {
  ...
  const setVisibility = (id: string, visibility: DocumentVisibility) => {
      setDocuments(prev => prev.map((file) => file.id === id ? {...file, visibility} : file));
  };
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Add new documents to your profile</p>
          </label>
        </div>
                ...
                <div key={file.id} className="bg-background dark:bg-dark-surface p-3 rounded-lg flex items-center justify-between animate-fade-in-up border border-border dark:border-dark-border hover:shadow-md hover:border-primary/50 transition-all">
                    ...
                    <div className="flex items-center gap-2 ml-2">
                        <CustomSelect<DocumentVisibility>
                            ...
                            onChange={(v) => setVisibility(file.id, v)} 
                        ...
                        <button onClick={() => handleDelete(file.id)} className="p-1.5 text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition-colors rounded-full hover:bg-slate-200 dark:hover:bg-slate-700" title="Delete"><TrashIcon /></button>
                    </div>
                </div>
            )) : (
